...
// 获取翻译后的提示词
async function getPrompt(prompt: string, model: string): Promise<string> {
  const requestBody = {
    messages: [
      {
        role: "system",
...
// 获取 Flux 模型的翻译后的提示词
async function getFluxPrompt(prompt: string, model: string): Promise<string> {
  const requestBody = {
    messages: [
      {
        role: "system",
...
提示词：A blurry polaroid of a 1980s living room, with vintage furniture, soft pastel tones, and a nostalgic, grainy texture,  The sunlight filters through old curtains, casting long, warm shadows on the wooden floor, 1980s,
...
// 从外部API获取提示词
async function getExternalPrompt(requestBody: any): Promise<string> {
  try {
    ...
    const jsonResponse = await response.json() as { choices: { message: { content: string } }[] };
...
// 从Cloudflare获取提示词
async function getCloudflarePrompt(model: string, requestBody: any): Promise<string> {
...
// 生成图像并存储到 KV
async function generateAndStoreImage(model: string, prompt: string, requestUrl: string): Promise<string> {
  try {
...
async function generateAndStoreFluxImage(model: string, prompt: string, requestUrl: string): Promise<string> {
  try {
    ...
    throw new Error("Flux图像生成失败: " + (error as Error).message);
  }
}

// 使用 Kolors 模型生成并存储图像
async function generateAndStoreKolorsImage(model: string, prompt: string, requestUrl: string): Promise<string> {
  try {
    ...
    throw new Error("Kolors图像生成失败: " + (error as Error).message);
  }
}
...
// 生成响应内容
function generateResponseContent(originalPrompt: string, translatedPrompt: string, size: string, model: string, imageUrl: string, promptModel: string): string {
...
// 调用ai env
async function postRequestEnv(model: string, jsonBody: any): Promise<any> {
  const response = await CONFIG.CF_ENV?.AI.run(model as keyof AiModels, jsonBody);
...
// 发送POST请求
async function postSfRequest(model: string, prompt: string, height: number, width: number): Promise<ReadableStream> {
...
// 处理流式响应
function handleStreamResponse(originalPrompt: string, translatedPrompt: string, size: string, model: string, imageUrl: string, promptModel: string): Response {
...
// 处理非流式响应
function handleNonStreamResponse(originalPrompt: string, translatedPrompt: string, size: string, model: string, imageUrl: string, promptModel: string): Response {
  const content = generateResponseContent(originalPrompt, translatedPrompt, size, model, imageUrl, promptModel);
  const response = {
    id: `chatcmpl-${Date.now()}`,
